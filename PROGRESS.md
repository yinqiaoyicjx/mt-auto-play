...
| 2025-02-13 | 地图区域检测修复 | ✅ 通过 | 排除左右状态栏 |
| 2025-02-13 | 策略优先级调整 | ✅ 通过 | 换楼层提升到优先级2 |
| 2025-02-13 | 钥匙验证逻辑改进 | ✅ 通过 | 验证可达性，距离限制 |
| 2025-02-13 | 窗口激活功能 | ⚠️ 网页游戏需手动聚焦 | 网页游戏不支持自动激活 |
| - | 实际游戏测试 | ⏳ 待测试 | 需要游戏运行 |
...
- ✅ 修复：地图区域检测（排除状态栏）
  - config.py: 添加MAP_REGION配置（140-558px）
  - detector.py: 所用检测方法仅在地图区域检测
  - 解决"检测到状态栏钥匙"问题

- ✅ 优化：策略优先级调整
  - planner.py: 换楼层从优先级5提升到优先级2
  - 改进钥匙收集：验证可达性
  - 距离>20步的钥匙优先考虑换楼层
  - 不可达钥匙自动清除

- ✅ 新增：窗口激活功能
  - controller.py: 添加activate_window()方法
  - 支持win32窗口API自动激活
  - 网页游戏需要手动聚焦（已注释自动激活）
